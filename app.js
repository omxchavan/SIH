...
app.get("/map", async (req, res) => {
  try {
    ...
    res.render("map", {
      trains: [],
      error:
        "Unable to connect to train API. Please check your API key and network connection.",
    });
  }
});
...
app.get("/analysis/:trainNumber", async (req, res) => {
  ...
  try {
    const response = await axios.get(
      `https://railradar.in/api/v1/trains/${trainNumber}`,
      {
        headers: {
          "x-api-key": API_KEY,
        },
    ...
    if (response.data.success) {
      ...
      res.render("analysis", {
        trainData: null,
        error:
          response.data.error?.message || "Failed to fetch train analysis data",
        searchedTrain: trainNumber,
        searchedDate: journeyDate,
      });
    }
    ...
    res.render("analysis", {
      trainData: null,
      error:
        "Unable to fetch train data. Please check train number and try again.",
      searchedTrain: trainNumber,
      searchedDate: journeyDate,
    });
  }
